...
const getSimulatedResponse = (prompt: string, neighborhood: string = 'GENERAL'): string => {
  ...
  if (
    ...
    return "Ofertas Laborales Urgentes (Miércoles 3 Mar) 💼:\n\n" +
      ...
      "• ¿Buscas trabajo? Mañana lunes Tarragona Impulsa atiende de 9h a 14h.";
  }
  ...
  if (p.includes("colegio") || p.includes("escuela") || p.includes("escola") || p.includes("instituto") || p.includes("matricula") || p.includes("matrícula") || p.includes("calendario escolar") || p.includes("menu escolar") || p.includes("menú")) {
    return "🎓 INFORMACIÓN ESCOLAR (Domingo 15 Mar):\n\n📅 **JORNADA:**\n• Hoy domingo: Sin clases. Descanso escolar.\n• Mañana lunes: Vuelta a la rutina escolar con normalidad.\n• Menú: Consultar en la web del centro.\n• Extraescolares: Reinician mañana lunes.\n\n💡 Recordad los talleres de Tarragona Impulsa para familias.";
  }
  ...
  if (p.includes("noticia") || p.includes("pasando") || p.includes("hoy") || p.includes("evento") || p.includes("viento") || p.includes("tiempo") || p.includes("plan")) {
    return "Resumen para el domingo 15 de marzo ☀️:\n\n• 🏛️ PATRIMONIO: Entrada reducida al Anfiteatro y Museo Arqueológico hoy.\n• 🎞️ CINE: Parásitos V.O. en el Metropol a las 20:00h.\n• ☀️ TIEMPO: Día soleado con máximas de 18°C.\n• 🚌 EMT: Horario de domingos y festivos en todas las líneas.";
  }
  ...
  // 9. Nàstic y Actualidad
  if (p.includes("nastic") || p.includes("futbol") || p.includes("caballos")) {
    return "¡Actualidad del Barrio! 🐎⚽\n\n• El Nàstic jugó el fin de semana, consulta el resultado en el Diari. ¡Próximo partido fuera!\n• El taller de LinkedIn de ayer lunes fue un éxito de participación. ¡Hoy seguimos con Networking!";
  }
