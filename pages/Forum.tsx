...
const Forum: React.FC = () => {
  ...
  const tickerMessages = [
    { user: 'Mireia R.', text: '☀️ ¡Buen jueves! Hoy toca mercado en la Plaça del Fòrum. 🛒' },
  ...
  const conversationThreads: Record<string, Array<Array<{ who: string; text: string }>>> = {
    ...
    'APOYO': [
      ...
      [
        ...
        { who: 'Maria G.', text: '@Carme Sí, no puedo todos los días. Los lunes y jueves sí, pero necesitaría que alguien cubriera los viernes.' },
    ...
    'ENCUENTROS': [
      [
        { who: 'Joe R.', text: '🎾 ¿Hay alguien para una partida de pádel este jueves tarde? Somos 3, nos falta uno.' },
  ...
  useEffect(() => {
    if (!loading && seedsInjectedRef.current !== currentNeighborhood) {
      ...
      const seedsByChannel: Record<string, Message[]> = {
        'GENERAL': [
          { id: 'seed-wed-1', user_id: 'v3', content: '☀️ ¡Buenos días de jueves! Menudo solazo hace hoy. Ideal para ir al mercado. 🛒', user_metadata: { full_name: 'Joan B.', avatar_url: 'https://i.pravatar.cc/150?u=joan' }, neighborhood: 'GENERAL', created_at: new Date(now - 1000 * 60 * 22).toISOString() },
        ...
        'APOYO': [
          ...
          { id: 'seed-apoyo-9', user_id: 'v7', content: 'Me ofrezco como voluntaria para acompañar a personas mayores o a quien necesite compañía para ir a trámites, médico, etc. Tengo las tardes libres los lunes y jueves. Contactadme por aquí o al 644 22 33 88. No estamos solos. 💛', user_metadata: { full_name: 'Maria G.', avatar_url: 'https://i.pravatar.cc/150?u=maria' }, neighborhood: 'APOYO', created_at: new Date(now - 1000 * 60 * 5).toISOString() },
  ...
  const generateVirtualMessage = async (isReplyTo?: string, originalPrompt?: string, isChain?: boolean) => {
    ...
    // Base initiation scripts (Wednesday 26 Feb / Mid-week energy)
    let scripts = [
      "☀️ ¡Feliz jueves vecinos! ¿Quién va hoy al Mercado de la Part Alta? 🛒",
      ...
      "Jueves de mercado y cine. ¡Qué gran día para ser de Tarragona! 💪"
    ];

    // Base reply scripts
    let replyScripts = [
      `¡Totalmente, ${isReplyTo}! A por el jueves de mercado. 🧺`,
      `¡Buen jueves, ${isReplyTo}! Disfruta del sol. ☀️`,
      `¡Pasa rápido la semana, ${isReplyTo}! Ánimo con el jueves. 😄`,
      ...
      `¡Qué buena energía para un jueves! Gracias, ${isReplyTo}. 💪`
    ];

    if (currentNeighborhood === 'EMPLEO') {
      scripts = ["¿Alguien sabe de trabajos de tarde en hostelería esta semana? 🍽️", "Empiezo nuevo trabajo el jueves. ¡Un poco de ánimo! 🤞"];
      ...
      scripts = [
        ...
        "Recordatorio: grupo de apoyo emocional HOY jueves 17h en CC Torreforta. Gratuito y abierto a todos. 🫂",
      ...
      scripts = [
        "☀️ ¡Feliz jueves! ¿Quién se apunta a un café rápido antes de ir al mercado? ☕",
        "Busco gente para completar partido de pádel HOY tarde en el polideportivo. 🎾",
        "Jueves, mitad de semana... ¿quién necesita un plan para desconectar? 💪",
        ...
        "¡Hola! ¿Hay algún plan tranquilo para este jueves noche? 👋"
      ];
      replyScripts = [
  ...
  const trendingTopics = [
    ...
    },
    {
      ...
      description: 'Jueves y jueves Forum.',
      participating: 2210
    },
    {
      id: 'empleo-semana',
